...
- [x] T017 [P] [US1] Add `scripts/check-greeting-variation.mjs`: calls the proxy's `/chat/completions` endpoint 10 times with only the system message (greeting trigger per research §1 / contracts/ai-proxy.md), counts unique greetings, exits 0 iff ≥ 8/10 are unique. Acceptance: SC-009 closed (script run produces "8/10 unique" or higher). blockedBy: T005 — code-reviewer APPROVED 2026-04-29
