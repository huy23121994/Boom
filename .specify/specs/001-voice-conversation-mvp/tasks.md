...
- [x] T008 [P] [US1] Implement `src/lib/vad.ts` (lazy-load `@ricky0123/vad-web` `MicVAD.new()` on first call to a `startVAD(callbacks)` function; expose `stop()`; surface `onSpeechStart` / `onSpeechEnd` callbacks). Install `@ricky0123/vad-web` as a dependency. Plus `src/lib/vad.test.ts` (mock the dynamic-import boundary; assert MicVAD is not constructed before first call; assert callbacks wire through). Acceptance: provides the VAD endpoint signal the orchestrator uses for FR-009; matches research §5. blockedBy: T002
...
- [ ] T014 [P] [US4] Implement `src/components/SettingsDrawer.tsx` (top-right small low-emphasis icon button → drawer; opening the drawer does **not** interrupt an active turn — i.e., does not dispatch any state-machine action). Drawer contains: (a) the persistence toggle ("Save conversations to this device"), (b) "Clear all history" button, and (c) `<ThemeToggle />` (separate component below). Style with UnoCSS utility classes including the design-token shortcuts (`bg-elevated`, `text-default`, `border-default`). Wire `togglePersist` and `clearAll` actions per the trigger-path matrix in `contracts/storage-adapter.md`. Also implement `src/components/ThemeToggle.tsx` (research §17): a small button that displays the current theme (light/dark/system icon) and on click calls `setTheme(next)` cycling `light → dark → system → light`; consumes `useTheme()` selector. Plus `src/components/SettingsDrawer.test.tsx` (opening the drawer leaves `bubbleState` unchanged; toggling persistence ON→OFF calls `clearTranscript()` then `savePrefs(...)`; toggling OFF→ON calls `savePrefs(...)` then `saveTranscript(...)`; "Clear all history" calls `clearAll()` then `savePrefs({persistEnabled:false, theme:<current>, version:1})` and dispatches `clearAll`) and `src/components/ThemeToggle.test.tsx` (clicking cycles light → dark → system → light; each click dispatches `setTheme` once with the next mode; the icon rendered matches the current mode). Acceptance: FR-018, FR-018a closed; SC-002/SC-003 (drawer leg) closed; theme cycle covered. blockedBy: T009, T010
...
- [ ] T022 Finalize `vite-plugin-pwa` configuration in `vite.config.ts`: full `manifest` (name `Boom — Daily English Tutor`, `short_name: 'Boom'`, `display: 'standalone'`, `theme_color` and `background_color` set to the bubble palette finalized in T012's `index.css`); `workbox.runtimeCaching` adds `NetworkOnly` for the AI proxy origin and `CacheFirst` for the vad-web onnx model URL; `workbox.globPatterns` precaches the app shell. Acceptance: built `dist/manifest.webmanifest` matches research §6; service worker excludes the AI proxy origin from caching (verified by inspecting the generated `sw.js`). blockedBy: T003, T012
- [ ] T023 Add `public/icons.svg` (single source) and configure `vite-plugin-pwa` (or a simple build script) to emit `icons/192.png` and `icons/512.png` referenced by the manifest. Acceptance: `dist/icons/192.png` and `dist/icons/512.png` are produced by `npm run build`; the manifest references both with correct `sizes` attributes. blockedBy: T022
